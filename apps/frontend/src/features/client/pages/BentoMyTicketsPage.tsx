        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
                    ...
                    <div>
                        <div className="flex items-center gap-2">
                            ...
                            <div className={cn(
                                ...
                                <span className={cn("w-1.5 h-1.5 rounded-full", isConnected ? "bg-green-500 animate-pulse" : "bg-slate-400")} />
                ...
                <div className="glass-card hover:glass-hover-lift rounded-2xl p-4 transition-all duration-200">
                    <div className="flex items-center gap-3">
                        <div className="w-12 h-12 rounded-xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center">
                            {React.createElement(STATUS_CONFIG['TODO']?.icon || Inbox, { className: "w-6 h-6 text-slate-600 dark:text-slate-400" })}
                        </div>
                        <div>
                ...
                <div className="glass-card hover:glass-hover-lift rounded-2xl p-4 transition-all duration-200">
                    <div className="flex items-center gap-3">
                        <div className="w-12 h-12 rounded-xl bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center">
                            {React.createElement(STATUS_CONFIG['IN_PROGRESS']?.icon || CircleDot, { className: "w-6 h-6 text-blue-600 dark:text-blue-400" })}
                        </div>
                        <div>
                ...
                <div className="glass-card hover:glass-hover-lift rounded-2xl p-4 transition-all duration-200">
                    <div className="flex items-center gap-3">
                        <div className="w-12 h-12 rounded-xl bg-green-100 dark:bg-green-900/30 flex items-center justify-center">
                            {React.createElement(STATUS_CONFIG['RESOLVED']?.icon || CheckCircle2, { className: "w-6 h-6 text-green-600 dark:text-green-400" })}
                        </div>
                        <div>
            ...
            <div className="glass-card overflow-hidden rounded-2xl">
                ...
                <div className="p-4 border-b border-white/20 dark:border-white/10 flex flex-col md:flex-row gap-4 bg-white/40 dark:bg-slate-900/40 backdrop-blur-md">
                    ...
                    <div className="flex gap-2 font-medium" role="group">
                        {STATUS_FILTERS.map((filter, index) => (
                            <button
                                ...
                                className={`px-4 py-2 rounded-xl text-xs font-bold transition-colors duration-150 flex items-center gap-2 ${statusFilter === filter.value
                            ...
                            >
                                {filter.value !== 'all' && STATUS_CONFIG[filter.value]?.icon && 
                                    React.createElement(STATUS_CONFIG[filter.value].icon as any, { className: "w-3.5 h-3.5" })
                                }
                                {filter.label}
                            </button>
                        ))}
                ...
                <div className="divide-y divide-white/20 dark:divide-white/10">
                    {tickets.length === 0 ? (
                        <div className="p-16 text-center bg-white/30 dark:bg-slate-800/30 backdrop-blur-sm">
                            ...
                            <Link
                                to="/client/create"
                                className="inline-flex items-center gap-2 bg-primary text-slate-900 px-6 py-3 rounded-xl font-bold hover:shadow-xl hover:shadow-primary/30 hover:-translate-y-0.5 transition-all duration-200"
                            >
                                <Plus className="w-5 h-5" />
                                Create Ticket
                            </Link>
                        ...
                        tickets.map((ticket: TicketItem, index: number) => {
                            ...
                            try {
                                ...
                                if (diffInHours < 24) {
                                    ...
                                    timeDisplay = date.toLocaleDateString('en-GB', { month: 'short', day: 'numeric' });
                                }
                            } catch (e) {}
                                ...
                                >
                                    <div className="md:w-48 xl:w-56 flex items-center gap-3">
                                        <div className={cn("w-10 h-10 rounded-xl flex items-center justify-center shrink-0 shadow-sm", statusConfig.bgColor)}>
                                            {StatusIcon && <StatusIcon className={cn("w-5 h-5", statusConfig.textColor)} />}
                                        </div>
                                        <div>
                                    ...
                                    <div className="md:w-24 flex items-center justify-end gap-1 text-primary opacity-0 group-hover:opacity-100 transition-all duration-200 transform translate-x-[-10px] group-hover:translate-x-0">
                    ...
                    <div className="p-4 border-t border-white/20 dark:border-white/10 bg-white/30 dark:bg-slate-900/30 backdrop-blur-md flex flex-col sm:flex-row items-center justify-between gap-4">
                        <div className="flex items-center gap-2 text-xs text-slate-600 dark:text-slate-400 font-medium">
                            ...
                            <select
                                ...
                                className="px-2 py-1 rounded-lg bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 outline-none"
                            >
                        ...
                        <div className="flex items-center gap-1">
                            <button onClick={() => setPage(1)} disabled={!meta.hasPrevPage} className="p-1.5 rounded-lg hover:bg-white dark:hover:bg-slate-700 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"><ChevronsLeft className="w-4 h-4" /></button>
                            <button onClick={() => setPage(page - 1)} disabled={!meta.hasPrevPage} className="p-1.5 rounded-lg hover:bg-white dark:hover:bg-slate-700 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"><ChevronLeft className="w-4 h-4" /></button>
                            <span className="px-3 py-1 bg-primary/10 text-primary font-bold text-xs rounded-lg">{meta.page} / {meta.totalPages}</span>
                            <button onClick={() => setPage(page + 1)} disabled={!meta.hasNextPage} className="p-1.5 rounded-lg hover:bg-white dark:hover:bg-slate-700 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"><ChevronRight className="w-4 h-4" /></button>
                            <button onClick={() => setPage(meta.totalPages)} disabled={!meta.hasNextPage} className="p-1.5 rounded-lg hover:bg-white dark:hover:bg-slate-700 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"><ChevronsRight className="w-4 h-4" /></button>
                        </div>
                    </div>
                )}
            </div>
