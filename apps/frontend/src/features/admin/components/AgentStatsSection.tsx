...
const TopPerformerCard: React.FC<{ name: string; tickets: number; onClick?: () => void; isActive?: boolean }> = ({ name, tickets, onClick, isActive }) => (
    <div
        onClick={onClick}
        className={cn(
            "rounded-2xl p-5 border border-amber-400/30 bg-gradient-to-br from-amber-400 via-amber-500 to-orange-500 hover:shadow-lg hover:shadow-amber-500/20 transition-all hover:-translate-y-0.5 animate-golden-pulse cursor-pointer",
            isActive && "ring-2 ring-primary ring-offset-2 dark:ring-offset-slate-900"
        )}
    >
        <div className="flex items-center justify-between">
