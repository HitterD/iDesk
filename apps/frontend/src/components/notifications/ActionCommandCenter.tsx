...
const SectionLabel = ({ urgency, count }: { urgency: ActionItemUrgency; count: number }) => {
    ...
    return (
        <div className={`flex items-center gap-1.5 px-4 py-2 ${cfg.sectionBg}`}>
            <span className={`flex items-center gap-1 text-xs font-semibold ${cfg.sectionText}`}>
                {cfg.icon}
                {cfg.label}
            </span>
            <span className={`ml-auto text-xs font-medium ${cfg.sectionText} opacity-70`}>
    ...
    return (
        ...
        >
            ...
            <div className="flex-1 min-w-0">
                ...
                {item.isSnoozed && snoozeLabel && (
                    <button
                        ...
                        className="mt-1 text-xs text-slate-400 bg-slate-100 dark:bg-slate-700 rounded px-1.5 py-0.5 hover:bg-red-100 hover:text-red-500 dark:hover:bg-red-900/30 dark:hover:text-red-400 transition-colors"
                    >
            ...
            <div className="flex items-center gap-1 flex-shrink-0 mt-0.5">
                ...
                <span className="text-xs text-slate-400 dark:text-slate-500 hidden group-hover:hidden sm:block">{timeAgo(item.createdAt)}</span>
                    ...
                    {INTENSITIES.map(({ value, label, interval }) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className={`text-xs mt-0.5 ${active ? 'text-white/70' : 'text-slate-400 dark:text-slate-500'}`}>
                                    {interval}
                                </div>
                            </button>
                        );
    ...
    return (
        <div className="relative">
            ...
            >
                ...
                {/* Badge */}
                {activeTotal > 0 && (
                    <span className={`absolute top-1.5 right-1.5 min-w-[16px] h-4 px-0.5 flex items-center justify-center rounded-full border-2 border-white dark:border-slate-900 text-xs font-bold text-white ${
                        ...
                        >
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    {counts.total > 0 && (
                                        <div className="flex items-center gap-1 ml-1">
                                            {counts.critical > 0 && (
                                                <span className="text-xs font-semibold px-1.5 py-0.5 rounded-full bg-red-100 dark:bg-red-500/20 text-red-600 dark:text-red-400">
                                                    {counts.critical} kritis
                                                </span>
                                            )}
                                            {counts.high > 0 && (
                                                <span className="text-xs font-semibold px-1.5 py-0.5 rounded-full bg-amber-100 dark:bg-amber-500/20 text-amber-600 dark:text-amber-400">
                                                    {counts.high} tinggi
                                                </span>
                                            )}
                                        </div>
                            ...
                            <div className="px-4 py-2.5 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                                ...
                                <button
                                    ...
                                    className="text-xs text-slate-400 hover:text-primary dark:hover:text-primary transition-colors flex items-center gap-1"
                                >
