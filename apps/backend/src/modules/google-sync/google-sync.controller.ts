...
export class GoogleSyncController {
    ...
    async triggerSync(
        ...
        await this.scheduler.triggerSync(id, req.user?.userId);
    ...
    async syncAll(@Req() req: any) {
        const count = await this.scheduler.syncAll(req.user?.userId);
    ...
    async syncNow(
        ...
        const result = await this.syncEngine.syncSheet(id, req.user?.userId);
        return result;
    }
