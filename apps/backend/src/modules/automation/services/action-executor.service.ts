...
export class ActionExecutorService {
        ...
        try {
            ...
            switch (action.type) {
                ...
                case ActionType.CHANGE_ASSIGNEE:
                    result = await this.changeAssignee(ticket, action.config, ruleId);
                    break;
    ...
    async executeActions(
        ...
        for (const action of sortedActions) {
            const result = await this.executeAction(action, ticket, ruleId);
            results.push({
                type: action.type,
                ...result,
            });
    ...
    private async changeAssignee(
        ...
        switch (config.assignmentType) {
            ...
            case AssignmentType.ROUND_ROBIN:
                newAssigneeId = await this.getNextRoundRobinAgent(config.assignmentPool, ruleId);
                break;
    ...
    private async getNextRoundRobinAgent(pool: string[] | undefined, ruleId: string): Promise<string> {
        ...
        if (pool && pool.length > 0) {
            agents = await this.userRepo.findBy({ id: (id) => pool.includes(id as any) } as any);
        } else {
