...
export class IpWhitelistController {
     ...
     */
    @Patch(':id')
    async update(@Param('id') id: string, @Body() dto: UpdateIpWhitelistDto) {
        return this.ipWhitelistService.update(id, dto);
    }
    ...
    async remove(@Param('id') id: string) {
        const deleted = await this.ipWhitelistService.remove(id);
        return { deleted };
    }
}
