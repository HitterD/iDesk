...
export class WorkloadService {
    ...
    async getAllAgentWorkloads(siteId: string, date?: Date): Promise<AgentDailyWorkload[]> {
        const workDate = date || new Date();
        workDate.setHours(0, 0, 0, 0);
        ...
        const workloads: AgentDailyWorkload[] = [];
    ...
    async recalculateAgentWorkload(agentId: string, siteId: string): Promise<AgentDailyWorkload> {
    ...
    async findBestAgentForAssignment(siteId: string, excludeAgentIds: string[] = []): Promise<User | null> {
        ...
        // Get or create workload for each agent
        const agentWorkloads: { agent: User; points: number }[] = [];
        ...
        // Sort by points (ascending) - agent with lowest points first
        agentWorkloads.sort((a, b) => a.points - b.points);
