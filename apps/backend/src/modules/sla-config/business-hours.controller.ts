...
export class BusinessHoursController {
    ...
    async updateConfig(@Body() dto: UpdateBusinessHoursDto) {
        const config = await this.service.getDefault();
        const updated = await this.service.update(config.id, dto);
    ...
    async addHoliday(@Body() dto: AddHolidayDto) {
        const config = await this.service.getDefault();
        return this.service.addHoliday(config.id, dto.date);
    }
    ...
    async removeHoliday(@Param('date') date: string) {
        const config = await this.service.getDefault();
        return this.service.removeHoliday(config.id, date);
    }
