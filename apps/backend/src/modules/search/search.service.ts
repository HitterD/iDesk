...
export class SearchService {
    ...
    constructor(
        ...
        private readonly hardwareRequestRepo: Repository<HardwareRequest>,
    ...
    private async searchHardwareRequests(
        ...
        const qb = this.hardwareRequestRepo.createQueryBuilder('hw')
            .leftJoinAndSelect('hw.requester', 'requester');

        if (query && query.trim()) {
            const searchTerm = `%${query.trim().toLowerCase()}%`;
            qb.andWhere(new Brackets(qb => {
                qb.where('LOWER(hw.requestNumber) LIKE :search', { search: searchTerm })
                  .orWhere('LOWER(hw.justification) LIKE :search', { search: searchTerm });
            }));
        }

        if (filters.status?.length) {
            qb.andWhere('hw.status IN (:...status)', { status: filters.status });
        }

        qb.orderBy('hw.createdAt', 'DESC')
          .skip(offset)
          .take(limit);
        ...
        return requests.map(r => ({
            ...
            userName: r.requester?.fullName,
            highlight: query ? this.extractHighlight(r.justification, query) : undefined,
        }));
    }
    ...
    async getSuggestions(query: string, limit = 10): Promise<SearchSuggestionDto[]> {
        ...
        // Hardware Requests
        const hwRequests = await this.hardwareRequestRepo
            .createQueryBuilder('hw')
            .where('LOWER(hw.requestNumber) LIKE :search', { search: searchTerm })
            .orWhere('LOWER(hw.justification) LIKE :search', { search: searchTerm })
            .take(3)
            .getMany();
        ...
        hwRequests.forEach(r => {
            suggestions.push({
                text: `${r.requestNumber}: ${r.justification?.slice(0, 50)}`,
