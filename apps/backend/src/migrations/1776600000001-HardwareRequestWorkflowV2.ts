...
export class HardwareRequestWorkflowV21776600000001 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`
      ALTER TABLE hardware_request_items
    ...
    await queryRunner.query(`
      CREATE INDEX IF NOT EXISTS idx_hri_delivery_status
        ON hardware_request_items(delivery_status)
    `);
    ...
    await queryRunner.query(`
      CREATE TABLE IF NOT EXISTS installation_schedule_items (
        ...
        schedule_id uuid NOT NULL REFERENCES installation_schedules(id) ON DELETE CASCADE,
        item_id uuid NOT NULL REFERENCES hardware_request_items(id) ON DELETE CASCADE,
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`
      ALTER TABLE installation_schedules
        ADD CONSTRAINT installation_schedule_status_check
          CHECK (status::text IN ('PROPOSED','CONFIRMED','IN_PROGRESS','DONE','RESCHEDULED','CANCELLED'))
    `);
    await queryRunner.query(`
      ALTER TABLE hardware_request_items
    ...
    await queryRunner.query(`
      ALTER TABLE hardware_requests
        ADD CONSTRAINT hardware_request_status_check
          CHECK (status::text IN ('DRAFT','SUBMITTED','UNDER_REVIEW','APPROVED','PROCUREMENT',
