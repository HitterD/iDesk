# Workload Page Redesign Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Redesign the Workload Command Center page to auto-select the first available site and replace the dropdown site selector with modern segmented controls (pill tabs).

**Architecture:** We will create a new component `SiteTabs.tsx` that fetches sites from the API and renders them as horizontal segmented controls. `AdminWorkloadDashboard.tsx` will be updated to use this new component, automatically select the first site on initial load, and completely remove the "Belum Ada Site Terpilih" empty state logic.

**Tech Stack:** React, Tailwind CSS, Lucide Icons, existing `workloadApi` and `api` utility.

---

### Task 1: Create SiteTabs Component

**Files:**
- Create: `apps/frontend/src/components/site/SiteTabs.tsx`

- [ ] **Step 1: Write the minimal implementation for SiteTabs**
...
```bash
git add apps/frontend/src/components/site/SiteTabs.tsx
git commit -m "feat(ui): add segmented controls SiteTabs component for workload redesign"
```

### Task 2: Update AdminWorkloadDashboard

**Files:**
- Modify: `apps/frontend/src/features/manager/pages/AdminWorkloadDashboard.tsx`

- [ ] **Step 1: Replace SiteSelector with SiteTabs and remove empty state**

Replace imports to include `SiteTabs` instead of `SiteSelector`.
Change `selectedSites` state from an array `string[]` to a single string `string`.
Remove the empty state UI since `SiteTabs` handles auto-selection.
...
```bash
git add apps/frontend/src/features/manager/pages/AdminWorkloadDashboard.tsx
git commit -m "refactor(workload): use SiteTabs and implement auto-selection on init"
```
